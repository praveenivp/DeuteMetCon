...
fn_fm=(fullfile(sn,dirst_me(end).name));
...
for cf=1:length(dirst_me)
    ...
    mcobj_me{cf}.setRefereneceVoltage(606);
%     mcobj_me{cf-1}=MetCon_bSSFP(fn,ME_setting{:},'fm','M01169_piv_gre_B0mapping_5Echoes_fmap.nii','Solver','pinv');
end
% mcobj_me{2}.ShiftMetcon([0,-10,0])% shift 7.5mm in read for up in IS direction
...
for cf=1:length(dirst_csi)
    ...
    mcobj_csi{cf}.setRefereneceVoltage(606);
end
% mcobj_csi{2}.ShiftMetcon([0,0,-10]) % shift in slice for up in IS direction

%%
cd(pn)
intake_time_me=cellfun(@(x) x.getMinutesAfterIntake('10:10'),mcobj_me,'UniformOutput',true);
ylabel_str_me=strsplit(sprintf('%.0f min\n',intake_time_me(:)),'\n');

intake_time_csi=cellfun(@(x) x.getMinutesAfterIntake('10:10'),mcobj_csi,'UniformOutput',true);
...
%
[imComposed_csi]=ComposeImage({met_mm_me_sag(:,:,:,:,[1 3]),met_mm_me_tra(:,:,:,:,[1 3])}, ...
...
tt=tiledlayout(6,3,'TileSpacing','compact','Padding','compact');
...
cax_met{1}=[0 50]
[imComposed_me]=ComposeImage({met_mm_me_sag(:,:,:,:,[2 4]),met_mm_me_tra(:,:,:,:,[2 4])}, ...
...
nexttile(tt,[2 1])
sf=ones (size(type_label_s2));
sf(type_label_s2==3)=SNR_scl;
violin(squeeze(col_tra(mask(:),1,:)).*sf,violin_format{:},'facecolor',colors_label(type_label_s2,:));
grid on, title(title_str{1}),xticklabels(intake_time_all),set(gca,'FontWeight','bold')

mean_val=mean(squeeze(col_tra(mask(:),1,:)).*sf);
...
mean_val=mean(squeeze(col_tra(mask(:),2,:)).*sf);
hold on,plot(find(type_label_s2==1),mean_val(type_label_s2==1),'Marker','x','Color',colors_label(1,:),'LineWidth',1.5)
hold on,plot(find(type_label_s2==3),mean_val(type_label_s2==3),'Marker','x','Color',colors_label(3,:),'LineWidth',1.5)
...
mean_val=mean(squeeze(col_tra(mask(:),3,:)).*sf);
hold on,plot(find(type_label_s2==1),mean_val(type_label_s2==1),'Marker','x','Color',colors_label(1,:),'LineWidth',1.5)
hold on,plot(find(type_label_s2==3),mean_val(type_label_s2==3),'Marker','x','Color',colors_label(3,:),'LineWidth',1.5)
