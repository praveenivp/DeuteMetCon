...
nexttile(tt,[2 1])
SNR_scl_vec=ones(size(type_label_s2));
SNR_scl_vec(type_label_s2==3)=SNR_scl;
violin(squeeze(col_tra(mask(:),1,:)).*SNR_scl_vec,violin_format{:},'facecolor',colors_label(type_label_s2,:));
grid on, title(title_str{1}),xticklabels(intake_time_all),set(gca,'FontWeight','bold')

mean_val=mean(squeeze(col_tra(mask(:),1,:)).*SNR_scl_vec);
