...
for cTR=1:size(sig_all_bssfp,1)
    ...
    for CM=1:size(sig_all_bssfp,3)
        ...
        [Msig_all,dc_fac_ssfp(cTR,1,CM)]=MetSignalModel(metabolites(CM),TE,deg2rad(pc_range), ...
            TR_all_bssfp(cTR),-1*metabolites(CM).freq_shift_Hz,deg2rad(FA_all),'bSSFP');
        sig_all_bssfp(cTR,:,CM)=abs(mean(abs(Msig_all),3));
        %            nexttile(), plot(pc_range,abs(Msig_all(:)))
    end
end
% DC=(TR_all_bssfp(:)-4e-3)./TR_all_bssfp(:); % 4ms non-encoding-time
% DC(DC<0)=0;
...
for cTR=1:size(sig_all_gre,1)
    ...
    for CM=1:size(sig_all_gre,3)
        ...
        [Msig_all,dc_fac_fisp(cTR,1,CM)]=MetSignalModel   (metabolites(CM),TE,0, ...
            TR_all_gre(cTR),-1*metabolites(CM).freq_shift_Hz*0,deg2rad(FA_all),'FISP',DC);
        sig_all_FISP(cTR,:,CM)=mean(abs(Msig_all),3);

        [Msig_all,dc_fac_gre(cTR,1,CM)]=MetSignalModel   (metabolites(CM),TE,0, ...
            TR_all_gre(cTR),-1*metabolites(CM).freq_shift_Hz*0,deg2rad(FA_all),'FLASH',DC);
...
fh=figure(3);
set(fh,'OuterPosition',[72 42 1355 1073]),clf
tt=tiledlayout(4,4,"TileSpacing","compact",'Padding','compact');
for i=1:length(metabolites)
    ...
    axis square
    set(gca,'clim',round(get(gca,'clim').*[0 1],1))
