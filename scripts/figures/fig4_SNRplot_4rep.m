...
imSz=size(realigned_vol);
realigned_vol=sum(reshape(realigned_vol(:,:,:,:,:),[imSz(1:end-1),3,4]),6)./sqrt(4);
realigned_vol=realigned_vol(:,:,:,:,[2 1 3]);
data_label={'CSI','CSI-PC-SSFP','ME-PC-bSSFP'};
label_ID=[2 1 3 2 1 3 2 1 3 2 1 3];
%%
im_sf=3;
realigned_vol3=NDimscale(realigned_vol,im_sf);

vox_vol=mean(cellfun(@(x)prod(x.DMIPara.resolution_PSF(1:3)*1e2),mcobj_all,'UniformOutput',true),1);
vox_vol=vox_vol./vox_vol(1);
...
%% get ROIs and plot
slcSel=25*im_sf+(-5:5);%+(-20:-15); % more slices for better stats
getRotm= @(theta)[cosd(theta) sind(theta); -sind(theta) cosd(theta)];
tubes={[38,113],[113,48]}; %diagonally opposite 9th tube and 
% tubes={[13,38],[38,16]}; %diagonally opposite 9th tube and 
...
for ii=1:size(realigned_vol,4)
    ...
    % realigned_vol4(:,:,slcSel,:,:)=max(realigned_vol4,[],3);%maximum intensity projection
...
cMask=imdilate(cMask,strel('disk',round(3*im_sf)),5);
cMet=reshape(mean(realigned_vol4(:,:,slcSel,ii,:),30),numel(cMask)*length(slcSel),[]);
cMet=cMet(cMask,:);
...
fontsize(gcf,'scale',1.5);
    ...
    %propagate std
std_ratio=(stat_std./stat_mean).^2;
std_ratio= sqrt(std_ratio(:,2:3)+std_ratio(:,1));
mean_ratio=stat_mean(:,2:3)./(stat_mean(:,1));

cell2table(arrayfun(@(x,y)sprintf('%.2f±%.2f',x,y),mean_ratio,std_ratio,'UniformOutput',false))
