...
[rf, gz, gzReph] = mr.makeSincPulse(FA, 'Duration', pulse_dur,...
    ...
    'system' ,system);
TE=pulse_dur/2+mr.calcDuration(gzReph)+1e-3; %[s]
%
crusherx=mr.makeTrapezoid('x','amplitude',20e-3*system.gamma,'duration',10e-3);
crushery=mr.makeTrapezoid('y','amplitude',20e-3*system.gamma,'duration',10e-3);
crusherz=mr.makeTrapezoid('z','amplitude',20e-3*system.gamma,'duration',10e-3);
...
%         seq.addBlock(rf);
        time_until_adc= TE-mr.calcDuration(rf)/2-mr.calcDuration(gzReph);
