classdef MetSim < matlab.mixin.Copyable
    ...
    methods
        ...
        function getSig(obj)
            ...
            obj.FieldMap_Hz= obj.flags.B0Range*(getRandomFmap(obj,[obj.flags.MatSize; obj.flags.MatSize;  1]));
        ...
        function B0map = getRandomFmap(~,imSize)
            ...
            % Adjust the variance range to control the shape of the histogram
            minVariance = 0.01;  % minimum variance
            maxVariance = 0.1;   % maximum variance
            varG = minVariance + (maxVariance - minVariance) * rand([3, Ncomp]);

            % Adjust the range of meanG for variability
            meanG = (rand([3, Ncomp]) - 0.5) * 2;  % Mean range: -1 to 1
        ...
        function getFlags(obj,varargin)
            ...
            addParameter(p,'NoiseFac',0.01,@(x) isscalar(x));
            % max- min values in random fieldmap
            addParameter(p,'B0Range',1,@(x) isscalar(x)); % Hz
