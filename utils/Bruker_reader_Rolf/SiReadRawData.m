function [raw, corr, flags, Mdhs,params] = SiReadRawData(path,channel,average,removeemptyedges)
% Reads Siemens raw data files in either VB or VE versions
% Called: e.g. [raw, corr, flags, Mdhs] = SiReadRawData(filename,[],1,0);
% removeemptyedges: Removes all lines/partitions/  that are empty at the edges of the k space. Mainly for weighted CSI, where there may be empty lines at the edges
VD = false;
corr = struct;
flags = 0;
if nargin < 4
...
if length < 1000
    ...
    %c = ftell(file)
    if NumScans == 1
        [Mdhs, Scans, Samples] = SiReadallMdhsVEU(file,DataLength);
    else
...
for cnt = 1:nMdhs*NChannels
    ...
    fprintf('Reading data: %3d %% done.',0)
    for cnt = 1:nMdhs
            ...
            elseif bitget(Mdhs(MdhInd).flags,26)
                ...
                if average == 0
                    ...
                    if Mdhs(cnt).ISet == 0 && Mdhs(cnt).IAcq == 0
                        raw(:,Mdhs(cnt).ILine+1-minLine, Mdhs(cnt).IPartition+1-minPar,Mdhs(cnt).ISlice+1,Mdhs(cnt).IEcho+1,Mdhs(cnt).IPhase+1,Mdhs(cnt).IMeas+1,Mdhs(cnt).ISegment+1-minSeg,currchannel)=dat;  % + 1e-5;    % Remove this!!!;
                        flags(Mdhs(cnt).ILine+1-minLine, Mdhs(cnt).IPartition+1-minPar,Mdhs(cnt).ISlice+1,Mdhs(cnt).IEcho+1,Mdhs(cnt).IPhase+1,Mdhs(cnt).IMeas+1,Mdhs(cnt).ISegment+1-minSeg,Mdhs(cnt).IAcq+1,Mdhs(cnt).ISet+1,currchannel)=Mdhs(cnt).flags;
                    else
                       % if mean(abs(dat(1:20)))>5e-6
                       %     Mdhs(cnt).IPartition
                       % end
                        raw(:,Mdhs(cnt).ILine+1-minLine, Mdhs(cnt).IPartition+1-minPar,Mdhs(cnt).ISlice+1,Mdhs(cnt).IEcho+1,Mdhs(cnt).IPhase+1,Mdhs(cnt).IMeas+1,Mdhs(cnt).ISegment+1-minSeg,currchannel)=raw(:,Mdhs(cnt).ILine+1-minLine, Mdhs(cnt).IPartition+1-minPar,Mdhs(cnt).ISlice+1,Mdhs(cnt).IEcho+1,Mdhs(cnt).IPhase+1,Mdhs(cnt).IMeas+1,Mdhs(cnt).ISegment+1-minSeg,currchannel)+dat;   % + 1e-5;    % Remove this!!!;
                        flags(Mdhs(cnt).ILine+1-minLine, Mdhs(cnt).IPartition+1-minPar,Mdhs(cnt).ISlice+1,Mdhs(cnt).IEcho+1,Mdhs(cnt).IPhase+1,Mdhs(cnt).IMeas+1,Mdhs(cnt).ISegment+1-minSeg,Mdhs(cnt).IAcq+1,Mdhs(cnt).ISet+1,currchannel)=Mdhs(cnt).flags;
                    end
                end
            end
