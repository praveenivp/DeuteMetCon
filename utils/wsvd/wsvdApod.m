...
[~,  wsvdQuality, wsvdCoilAmplitudes, wsvdWeights]  = wsvd(specApodize(timeAxis, rawSpectra, apod), [], 'noiseCov', noiseCov);

% Step 2: Apply weights to the raw single-element spectra.
wsvdApodCombination = rawSpectra * wsvdWeights;

end

function NC = normalise(C)
NC = conj(C)/norm(C);
...
for voxDx=1:prod(outputSize)
    ...
    [~,  wsvdQuality(:,voxDx), wsvdCoilAmplitudes(:,voxDx),   wsvdWeights(:,voxDx)]     = wsvd(...
