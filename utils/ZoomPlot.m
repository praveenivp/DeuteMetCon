...
set(figH,'ButtonDownFcn',toggle_selected,'HitTest','on')
...
while(rect_axis.Selected )
    ...
    if((~isequal(r2pos_old,rect_axis.Position) || ~isequal(r1pos_old,rect_zoom.Position)))
        ...
        if(strcmp(get(p_ax.YAxis,'Direction'),'reverse'))
             posax(2)=(p_ax.Position(4)-posax(2));
%              posax(2)=(((rect_axis.Position(2)-p_ax.YLim(1))/diff(p_ax.YLim)))*p_ax.Position(4)-p_ax.Position(2);
            ch_ax = axes('position',posax,'Selected','off','Layer','top','Box','on');
            set(ch_ax.YAxis,'Direction','reverse')
        else
        ch_ax = axes('position',posax,'Selected','off','Layer','top','Box','on');
...
%set(rect_axis,'Label','')
%copy color order
ch_ax.ColorOrder=p_ax.ColorOrder;
end
